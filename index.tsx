...
function MakeContextCallback(
    ...
    return type === "settings"
        ? (children, { guild, role }: { guild: Guild; role: Role }) => {
              children.splice(
                  -1,
                  0,
                  <Menu.MenuItem
                      ...
                      action={async () => {
                          createRole(
                              ...
                              role,
                              role.icon
                                  ? `https://cdn.discordapp.com/role-icons/${role.id}/${role.icon}.webp`
                                  : null
                          );
                      }}
                  />
        ...
        : (children, contextMenuApiArguments) => {
              ...
              const role = GuildStore.getRole(
                  guildid,
                  contextMenuApiArguments.id
              );
              if (!role) return;
              children.splice(
                  -1,
